...
$parentModule = 'ADMF'
if (-not $ModuleName) { $ModuleName = 'ADMF.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ADMF'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ADMF.psd1"
$prereqHash = @{
