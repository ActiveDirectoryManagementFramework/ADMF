...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'ADMF.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'ADMF.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'ADMF.Client.ProtectedToken'
